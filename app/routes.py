...
def index():
    ...
    return render_template('index.html', posts=posts, title='DevBlog Blog Personal')
...
def create_post():
    # Crear nuevo post (procesar formulario)
    # Maneja tanto GET (mostrar formulario) como POST
...
def search():
    ...
    return render_template('search.html', posts=results, query=query, message=message, title=f'Búsqueda: {query}' if query else 'Búsqueda DevBlog')
