...
class TestAPIEndpoints:
    ...
    def test_create_post_missing_data(self, client):
        ...
        response = client.post('/api/posts', data=json.dumps(incomplete_post), content_type='application/json')
    ...
    def test_create_post_invalid_json(self, client):
        ...
        response = client.post('/api/posts', data='{"title": "incomplete json"', content_type='application/json') # JSON malformado
        assert response.status_code == 400
    ...
    def test_update_post_success(self, client):
        ...
        response = client.put('/api/posts/1', data=json.dumps(update_data), content_type='application/json')
        assert response.status_code == 200
        assert response.content_type == 'application/json'
        data = json.loads(response.data)
        assert data['success'] is True
    ...
    def test_update_post_not_found(self, client):
        ...
        response = client.put('/api/posts/999', data=json.dumps(update_data), content_type='application/json')
        assert response.status_code == 404
        data = json.loads(response.data)
        assert data['success'] is False
